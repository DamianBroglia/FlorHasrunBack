...
const postComment = async (userName, date, comment, rating, productId, userId) => {
    ...
    if (userId && userName && rating && comment && date) {
        const postComment = await Comment.create({ userName, date, comment, rating, productId, userId })
        return postComment;
    }
